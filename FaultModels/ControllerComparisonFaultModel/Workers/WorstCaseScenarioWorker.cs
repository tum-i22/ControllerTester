...
namespace FM4CC.FaultModels.ControllerComparison
{
    internal class WorstCaseScenarioWorker : BackgroundWorker
    {
        ...
        private ControllerComparisonFaultModel fm;
                ...
                foreach (DataGridHeatPoint region in SelectedRegions)
                {
                    ...
                    if (!message.ToLower().Contains("success"))
                    {
                        ...
                        else
                        {
                            ...
                            Logger("Controller Comparison Fault Model - Accelerated worst case computation for " + region.Requirement + " failed. Unable to fit data to regressed model, falling back to doing simple Simulated Annealing. Best regression function was " + bestRegressionFunction + " (RMSE: " + rmse.ToString() + ", Threshold: " + qualityThreshold.ToString() + ").");
                            fm.SetSearchParameters(requirementIndex, region.InitialDesiredRegion, region.FinalDesiredRegion, new ControllerComparisonHeatPoint(bestPoint[0,0], bestPoint[0,1], 0.0, 0.0, false, false), "SimulatedAnnealing");
                            message = (string)fm.Run("WorstCaseSearch");
                        }
                    }
                    else
                    {
                        ...
                        Logger("Controller Comparison Fault Model - Accelerated worst case computation successful for " + region.Requirement + ". Regression function " + chosenRegressionFunction + " selected (RMSE: " + rmse.ToString() + ", Threshold: " + qualityThreshold.ToString() + ").");
                    }
                    i++;
        ...
        {
            FaultModelTesterTestCase testCase = new ControllerComparisonFaultModelTestCase();
            testCase.FaultModel = faultModelName;
            testCase.Name = "Worst " + region.Requirement + " (comparison) in region (" +
