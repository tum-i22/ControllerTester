...
namespace FM4CC.FaultModels.ControllerComparison.Parsers
{
    internal static class SimpleHeatMapParser
    {
        internal static IList<HeatMapDataSource> Parse(string filePath, decimal rowFromValue, decimal rowToValue, int rows, decimal columFromValue, decimal columnToValue, int columns, IList<string> selectedRequirements)
        {
            ...
            retrievedData.Add(new HeatMapDataSource() { Name = "MeanDeviation", Rows = rows, Columns = columns, RowFromValue = rowFromValue, RowToValue = rowToValue, ColumnFromValue = columFromValue, ColumnToValue = columnToValue });
            retrievedData.Add(new HeatMapDataSource() { Name = "MaxDeviation", Rows = rows, Columns = columns, RowFromValue = rowFromValue, RowToValue = rowToValue, ColumnFromValue = columFromValue, ColumnToValue = columnToValue });

            IEnumerable<string> lines = System.IO.File.ReadLines(filePath);

            int cnt = 0;
            double[] maximumValue = new double[] { Double.NegativeInfinity, Double.NegativeInfinity, Double.NegativeInfinity, Double.NegativeInfinity, Double.NegativeInfinity, Double.NegativeInfinity, Double.NegativeInfinity };
            double[] minimumValue = new double[] { Double.PositiveInfinity, Double.PositiveInfinity, Double.PositiveInfinity, Double.PositiveInfinity, Double.PositiveInfinity, Double.PositiveInfinity, Double.PositiveInfinity };

            foreach (string line in lines)
            {
                if (cnt++ == 0) continue;
                string[] values = line.Split(',');
                bool exceededRange1 = Convert.ToBoolean(Int32.Parse(values[2]));
                bool exceededRange2 = Convert.ToBoolean(Int32.Parse(values[3]));

                for (int i = 0; i < 7; i++)
                {
                    double intensity = ObjectiveFunctionValueParser.Parse(values[4 * i + 4]);
                    double worstIntensity = ObjectiveFunctionValueParser.Parse(values[4 * i + 5]);

                    double x = Double.Parse(values[4*i + 6], CultureInfo.InvariantCulture);
                    double y = Double.Parse(values[4*i + 7], CultureInfo.InvariantCulture);
 
                    ControllerComparisonHeatPoint hp = new ControllerComparisonHeatPoint(x, y, intensity, worstIntensity, exceededRange1, exceededRange2);
