    ...
    internal class WorstCaseScenarioWorker : BackgroundWorker
    {
        ...
        private void singleStateWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                fm = e.Argument as SineFaultModel;
                ...
                foreach (DataGridHeatPoint region in SelectedRegions)
                {
                    ...
                    fm.SetSearchParameters(requirementIndex, region.FrequencyRegion, region.DesiredRegion, region.ContainedHeatPoint, (string)fm.FaultModelConfiguration.GetValue("OptimizationAlgorithm"));
                    ...
                    if (!message.ToLower().Contains("success"))
                    {
                        ...
                        else
                        {
                            ...
                            Logger("Sine Fault Model - Accelerated worst case computation for " + region.Requirement + " failed. Unable to fit data to regressed model, falling back to doing simple Simulated Annealing. Best regression function was " + bestRegressionFunction + " (RMSE: " + rmse.ToString() + ", Threshold: " + qualityThreshold.ToString() + ").");
                            fm.SetSearchParameters(requirementIndex, region.FrequencyRegion, region.DesiredRegion, new SineHeatPoint(bestPoint[0,0], bestPoint[0,1], 0.0, 0.0, false), "SimulatedAnnealing");
                            message = (string)fm.Run("WorstCaseSearch");
                        }
                    }
                    else
                    {
                        ...
                        Logger("Sine Fault Model - Accelerated worst case computation successful for " + region.Requirement + ". Regression function " + chosenRegressionFunction + " selected (RMSE: " + rmse.ToString() + ", Threshold: " + qualityThreshold.ToString() + ").");
                    }
                    i++;
                    this.ReportProgress((int)((double)i / SelectedRegions.Count * 100.0));
