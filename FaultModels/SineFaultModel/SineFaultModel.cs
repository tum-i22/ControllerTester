...
{
    public class SineFaultModel : FaultModel
    {
        private bool setupDone;
        private const string prefix = "CT_";
        private const string shortName = "SineFaultModel";
        private const string name = "Sinusoidal Desired Value";
        private const string description = "A sine wave signal is input to the controller in an attempt to turn it unstable, as well as checking its response to a sine wave input.";
        private string scriptsPath;

        #region Fault Model Implementation

        public SineFaultModel(ExecutionEnvironment executionEngine, string scriptsPath)
        {
            setupDone = false;
            this.scriptsPath = scriptsPath;
            this.ExecutionEngine = executionEngine;
            this.FaultModelConfiguration = new SineFaultModelConfiguration();
        ...
        public SineFaultModel(ExecutionEnvironment executionEngine, FaultModelConfiguration configuration, string scriptsPath)
        {
            setupDone = false;
            this.scriptsPath = scriptsPath;
            this.ExecutionEngine = executionEngine;
            this.FaultModelConfiguration = (SineFaultModelConfiguration)configuration;
            this.RandomExplorationWorker = new RandomExplorationWorker();
            this.WorstCaseWorker = new WorstCaseScenarioWorker();
            this.TestRunWorker = new TestRunWorker();
        }
        ...
        public override void SetUpEnvironment()
        {
            if (!setupDone)
            {
                ...
                ExecutionEngine.PutVariable(prefix + "SineFrequencyRangeStart", (double)0.0);
                ExecutionEngine.PutVariable(prefix + "SineFrequencyRangeEnd", (double)this.FaultModelConfiguration.GetValue("MaxFrequency"));
...
//            ExecutionEngine.ExecuteCommand("clear all;");
        ...
        public override bool Run(FaultModelTesterTestCase testCase)
        {
            ...
            SetTestRunParameters((double)input["Desired"], (double)input["Frequency"]);
        ...
        internal void SetTestRunParameters(double desired, double frequency)
        {
        ...
        private object ExecuteSimulationRun()
        {
            ...
            {               
                ...
                ExecutionEngine.PutVariable(prefix + "SineFrequency", frequency);
                ...
                return ExecutionEngine.ExecuteCommand("SineModelExecution");
            }
        ...
        private string ExecuteRandomExploration()
        {
            if (setupDone)
            {
                return ExecutionEngine.ExecuteCommand("RandomExploration_Sine");
            }
            else throw new InvalidOperationException("Setup not performed");
        }
        ...
        private object ExecuteWorstCaseSearch()
        {
            if (setupDone)
            {
                return ExecutionEngine.ExecuteCommand("SingleStateSearch_Sine");
            }
            else throw new InvalidOperationException("Setup not performed");
        }
