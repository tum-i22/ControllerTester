...
namespace FM4CC.FaultModels.Sine.Parsers
{
    internal static class SimpleHeatMapParser
    {
        internal static IList<HeatMapDataSource> Parse(string filePath, decimal rowFromValue, decimal rowToValue, int rows, decimal columFromValue, decimal columnToValue, int columns, IList<string> selectedRequirements)
        {
            ...
            foreach (string line in lines)
            {
                ...
                {
                    double intensity = ObjectiveFunctionValueParser.Parse(values[4 * i + 3]);
                    double worstIntensity = ObjectiveFunctionValueParser.Parse(values[4 * i + 4]);
                    ...
                    SineHeatPoint hp = new SineHeatPoint(x, y, intensity, worstIntensity, exceededRange);
