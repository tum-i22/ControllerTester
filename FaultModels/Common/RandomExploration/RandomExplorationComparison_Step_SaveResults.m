function [ ExplorationResults ] = RandomExplorationComparison_Step_SaveResults(DesiredValues, ObjectiveFunctionValues, LimitDesiredValues, LimitObjectiveFunctionValues, TotalRegions, PointsPerRegion, TempPath)
    % generates a file containing all the points found and a file
    % containing a processed view of the input space
    ExplorationResults = zeros(PointsPerRegion*TotalRegions+4, 11);
    ...
    for RegionCnt = 1 : TotalRegions
        for PointCnt = 1:PointsPerRegion
            ...
            ExplorationResults(PointCnt + PointsPerRegion * (RegionCnt-1), 10) = ObjectiveFunctionValues(RegionCnt, PointCnt, 8);
            ExplorationResults(PointCnt + PointsPerRegion * (RegionCnt-1), 11) = ObjectiveFunctionValues(RegionCnt, PointCnt, 9);
    ...
    for i = indexStart : indexStart + 3
        ...
        ExplorationResults(i, 10) = LimitObjectiveFunctionValues(iLim, 8);
        ExplorationResults(i, 11) = LimitObjectiveFunctionValues(iLim, 9);
    end
    
    ResultsFolderPath = strcat(TempPath, '\RandomExploration_Comparison');
    ...
    RandomExplorationResultsHeader={'InitialDesired,Desired,Stability,Precision,Smoothness,Responsiveness,Steadiness,PhysicalRangeExceeded,MeanDeviation,MaxDeviation,PhysicalRangeExceeded1,PhysicalRangeExceeded2'};
    ...
    RegionWorstValues = zeros(RegionsPerAxis, RegionsPerAxis, 7);
    RegionWorstIndexes = zeros(RegionsPerAxis, RegionsPerAxis, 7);
    
    RegionMeanValues = zeros(RegionsPerAxis, RegionsPerAxis, 7);
    RegionIndexes = zeros(RegionsPerAxis, RegionsPerAxis, 2);

    RegionPhysicalRangeExceeded = zeros(RegionsPerAxis, RegionsPerAxis,2);
    ...
    for RegionXCnt = 1:RegionsPerAxis
        for RegionYCnt = 1:RegionsPerAxis
            ...
            % check if physical range was exceeded in any controller
            if (ObjectiveFunctionValues(RegionYCnt+(RegionXCnt-1)*RegionsPerAxis, PointCnt, 8) == 1)
                RegionPhysicalRangeExceeded(RegionXCnt, RegionYCnt, 1) = 1;
            end
            
            if (ObjectiveFunctionValues(RegionYCnt+(RegionXCnt-1)*RegionsPerAxis, PointCnt, 9) == 1)
                RegionPhysicalRangeExceeded(RegionXCnt, RegionYCnt, 2) = 1;
            end
            
            % calculate mean and worst values
            for ObjectiveFncCnt = 1:7
                % init worst indexes
    ...
    for RegionCnt = 1 : TotalRegions        
        ...
        RegionResults(RegionCnt, 3) = RegionPhysicalRangeExceeded(RegionXCnt, RegionYCnt,1);
        RegionResults(RegionCnt, 4) = RegionPhysicalRangeExceeded(RegionXCnt, RegionYCnt,2);
        
        for ObjectiveFncCnt = 1 : 7
            RegionResults(RegionCnt, (ObjectiveFncCnt-1)*4 + 5) = RegionMeanValues(RegionXCnt, RegionYCnt, ObjectiveFncCnt);
            RegionResults(RegionCnt, (ObjectiveFncCnt-1)*4 + 6) = RegionWorstValues(RegionXCnt, RegionYCnt, ObjectiveFncCnt);
            RegionResults(RegionCnt, (ObjectiveFncCnt-1)*4 + 7) = RegionWorstIndexes(RegionXCnt, RegionYCnt, ObjectiveFncCnt, 1);
            RegionResults(RegionCnt, (ObjectiveFncCnt-1)*4 + 8) = RegionWorstIndexes(RegionXCnt, RegionYCnt, ObjectiveFncCnt, 2);
        end
    end
    
    RegionsFilePath = strcat(ResultsFolderPath, '\RandomExplorationRegions_Step.csv');
    RegionResultsHeader = {'RegionX,RegionY,RangeExceeded1,RangeExceeded2,Stability,StabilityWorst,StabilityWorstX,StabilityWorstY,Precision,PrecisionWorst,PrecisionWorstX,PrecisionWorstY,Smoothness,SmoothnessWorst,SmoothnessWorstX,SmoothnessWorstY,Responsiveness,ResponsivenessWorst,ResponsivenessWorstX,ResponsivenessWorstY,Steadiness,SteadinessWorst,SteadinessWorstX,SteadinessWorstY,MeanDeviation,MeanDeviationWorst,MeanDeviationWorstX,MeanDeviationWorstY,MaxDeviation,MaxDeviationWorst,MaxDeviationWorstX,MaxDeviationWorstY'};
