...
try
    ...
    assignin('base', CT_Compared_DesiredVariableName, CT_GenerateStepSignal(CT_SimulationSteps, CT_ModelTimeStep, 0, 0, CT_ModelSimulationTime));
    assignin('base', CT_Compared_DisturbanceVariableName, CT_GenerateConstantSignal(1, CT_SimulationSteps*CT_ModelTimeStep, 0));
    accelbuild(gcs);
    
    % pre-allocate space
    ObjectiveFunctionValues = zeros(CT_TotalRegions, CT_PointsPerRegion, 9, 3);    
    ...
    parfor RegionCnt = 1 : CT_TotalRegions
        ...
        CurrentObjectiveFunctionValues = zeros(CT_PointsPerRegion, 9, 3);
        ...
        for PointCnt = 1 : CT_PointsPerRegion
            ...
            CurrentObjectiveFunctionValues(PointCnt, :, :) = SimulateModelStepComparison(CT_ModelFile, CT_Compared_ModelFile, InitialDesiredValue, DesiredValue, CT_ActualValueRangeStart, CT_ActualValueRangeEnd, 0, CT_SimulationSteps, CT_ModelTimeStep, CT_DesiredVariableName, CT_Compared_DesiredVariableName, CT_ActualVariableName, CT_Compared_ActualVariableName, CT_DisturbanceVariableName, CT_Compared_DisturbanceVariableName, CT_TimeStable, CT_TimeStable, CT_SmoothnessStartDifference, CT_ResponsivenessClose, CT_AccelerationDisabled, CT_ModelConfigurationFile, CT_Compared_ModelConfigurationFile);
    ...
    LimitObjectiveFunctionValues = zeros(4, 9, 3);
    ...
    for LimitTestCases = 1 : 4
        LimitObjectiveFunctionValues(LimitTestCases, :, :) = SimulateModelStepComparison(CT_ModelFile, CT_Compared_ModelFile, LimitDesiredValuesInit(LimitTestCases,1), LimitDesiredValuesFinal(LimitTestCases,1), CT_ActualValueRangeStart, CT_ActualValueRangeEnd, 0, CT_SimulationSteps, CT_ModelTimeStep, CT_DesiredVariableName, CT_Compared_DesiredVariableName, CT_ActualVariableName, CT_Compared_ActualVariableName, CT_DisturbanceVariableName, CT_Compared_DisturbanceVariableName, CT_TimeStable, CT_TimeStable, CT_SmoothnessStartDifference, CT_ResponsivenessClose, CT_AccelerationDisabled, CT_ModelConfigurationFile, CT_Compared_ModelConfigurationFile);
    end  
    
    RandomExploration_Step_SaveResults(DesiredValues, ObjectiveFunctionValues(:,:,:,1), LimitDesiredValues, LimitObjectiveFunctionValues(:, :, 1), CT_TotalRegions, CT_PointsPerRegion, CT_TempPath);
    RandomExploration_Step_SaveResults(DesiredValues, ObjectiveFunctionValues(:,:,:,2), LimitDesiredValues, LimitObjectiveFunctionValues(:, :, 2), CT_TotalRegions, CT_PointsPerRegion, CT_Compared_TempPath);
    RandomExplorationComparison_Step_SaveResults(DesiredValues, ObjectiveFunctionValues(:,:,:,3), LimitDesiredValues, LimitObjectiveFunctionValues(:, :, 3), CT_TotalRegions, CT_PointsPerRegion, CT_TempPath);
