...
try
    ...
    RegionXStart = CT_SineFrequencyRangeStart + ((CT_SineFrequencyRangeEnd - CT_SineFrequencyRangeStart)/CT_Regions) * CT_RegionXIndex;
    RegionXEnd = CT_SineFrequencyRangeStart + ((CT_SineFrequencyRangeEnd - CT_SineFrequencyRangeStart)/CT_Regions) * (CT_RegionXIndex + 1);
    ...
    hSimulation = @(p)(-1*SimulateModelSine(CT_ModelFile, p(2), p(1), CT_SineAmplitude, CT_ActualValueRangeStart, CT_ActualValueRangeEnd, CT_MaxObjectiveFunctionIndex, CT_SimulationSteps, CT_ModelTimeStep, CT_DesiredVariableName, CT_ActualVariableName, CT_DisturbanceVariableName, CT_TimeStable, CT_TimeStable, CT_SmoothnessStartDifference, CT_ResponsivenessClose, CT_AccelerationDisabled, CT_ModelConfigurationFile));
    ...
    assignin('base', CT_DesiredVariableName, CT_GenerateSineSignal(CT_SimulationSteps, CT_ModelTimeStep, 0, 10, CT_SineAmplitude));
    ...
    if ~error
	    SingleStateSearch_Sine_SaveResults(p, objectiveFunctionValue, CT_TempPath);
